...
test('empty query', async ({ page }) => {
  ...
  await expect(page.getByRole('table')).toContainText('2556'); // Specimens
});
...
test('gender: male', async ({ page }) => {
  ...
  await page.goto('/search?query=' + encodeURIComponent(JSON.stringify(query)));
  await expect(page.getByRole('table')).toContainText('118'); // Patients
  await expect(page.getByRole('table')).toContainText('1190'); // Specimens
});
...
test('diagnosis: C50.%', async ({ page }) => {
  ...
  await page.goto('/search?query=' + encodeURIComponent(JSON.stringify(query)));
  // Single-digit counts can't be checked with toContainText, since any digit is a
  // substring of the surrounding table text. Assert the whole result row instead.
  const row = page.getByRole('row').filter({ hasText: 'proxy2' });
  await expect(row.getByRole('cell')).toHaveText(['', 'proxy2', '1', '5']); // Patients, Specimens
});
...
test('diagnosis age donor: <=30', async ({ page }) => {
  ...
  await page.goto('/search?query=' + encodeURIComponent(JSON.stringify(query)));
  await expect(page.getByRole('table')).toContainText('115'); // Patients
  await expect(page.getByRole('table')).toContainText('1141'); // Specimens
});
...
test('diagnosis age donor: 30-50', async ({ page }) => {
  ...
  await page.goto('/search?query=' + encodeURIComponent(JSON.stringify(query)));
  await expect(page.getByRole('table')).toContainText('101'); // Patients
  await expect(page.getByRole('table')).toContainText('998'); // Specimens
});
...
test('diagnosis age donor: >=50', async ({ page }) => {
  ...
  await page.goto('/search?query=' + encodeURIComponent(JSON.stringify(query)));
  await expect(page.getByRole('table')).toContainText('47'); // Patients
  await expect(page.getByRole('table')).toContainText('479'); // Specimens
});
...
test('diagnosis age donor: <= 2010-01-01', async ({ page }) => {
  ...
  await page.goto('/search?query=' + encodeURIComponent(JSON.stringify(query)));
  await expect(page.getByRole('table')).toContainText('155'); // Patients
  await expect(page.getByRole('table')).toContainText('1575'); // Specimens
});
...
test('diagnosis age donor: 2010-01-01 - 2015-01-01', async ({ page }) => {
  ...
  await page.goto('/search?query=' + encodeURIComponent(JSON.stringify(query)));
  await expect(page.getByRole('table')).toContainText('72'); // Patients
  await expect(page.getByRole('table')).toContainText('705'); // Specimens
});
...
test('diagnosis age donor: >= 2015-01-01', async ({ page }) => {
  ...
  await page.goto('/search?query=' + encodeURIComponent(JSON.stringify(query)));
  await expect(page.getByRole('table')).toContainText('29'); // Patients
  await expect(page.getByRole('table')).toContainText('276'); // Specimens
});
...
test('sample type: serum', async ({ page }) => {
  ...
  await page.goto('/search?query=' + encodeURIComponent(JSON.stringify(query)));
  await expect(page.getByRole('table')).toContainText('85'); // Patients
  await expect(page.getByRole('table')).toContainText('100'); // Specimens
});
...
test('sampling date: <= 2010-01-01', async ({ page }) => {
  ...
  await page.goto('/search?query=' + encodeURIComponent(JSON.stringify(query)));
  await expect(page.getByRole('table')).toContainText('154'); // Patients
  await expect(page.getByRole('table')).toContainText('1575'); // Specimens
});
...
test('storage temperature: liquid nitrogen', async ({ page }) => {
  ...
  await page.goto('/search?query=' + encodeURIComponent(JSON.stringify(query)));
  await expect(page.getByRole('table')).toContainText('179'); // Patients
  await expect(page.getByRole('table')).toContainText('348'); // Specimens
});
